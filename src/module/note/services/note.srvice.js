...
export const replacenote = async (req, res, next) => {
  try {
    ...
    const newnote = await Note.replaceOne(
      { _id: noteId },
      { ...req.body },
      { new: true }
    );
    ...
    return res
      .status(201)
      .json({ message: "note updated", updatedNote: newnote });
  } catch (error) {
    return res
      .status(500)
      .json({ message: "server error", error, message: error.message });
...
export const deletednote = async (req, res, next) => {
  try {
    const { authorization } = req.headers;
    const decoded = jwt.verify(authorization, "usertoken");
    const user = await User.findById(decoded.id);
    ...
    if (!note) {
     ...
     if (!note.userId) {
      return res.status(400).json({ message: "note does not have a user" });
    }
    if (note.userId.toString() !== decoded.id) {
      return res.status(404).json({ message: "not owner" });
    }
